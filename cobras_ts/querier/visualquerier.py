...
def update(bokeh_layout, xs, ys1, ys2, iteration, num_queries):
    topdiv = Div(
        text="<font size=\"15\"> <b>COBRAS<sup>TS</sup></b> </font>  <br><font size=\"2\">  # queries answered: " + str(
            num_queries) + "</font>", css_classes=['top_title_div'],
...
@gen.coroutine
def update_clustering(querier, bokeh_layout, bokeh_doc, data, clustering, cluster_indices, representatives, queries):
    ...
    for c, c_idxs, cluster_representatives in zip(clustering, cluster_indices, representatives):
        ...
        for i, repr_idx in enumerate(cluster_representatives):
            cluster_plot.line('x', 'y', source=dict(x=range(data.shape[1]), y=data[repr_idx, :]), line_width=2,
    ...
    bokeh_layout.children[1].children[1].children[1] = Div(
        text="""Click 'Show me more queries!' below to continue querying""", width=400, height=100)
...
class VisualQuerier(Querier):
    ...
    def query_points(self, idx1, idx2):
        ...
        self.bokeh_doc.add_next_tick_callback(
            partial(update, bokeh_layout=self.bokeh_layout, xs=list(range(self.data.shape[1])), ys1=self.data[idx1, :],
                    ys2=self.data[idx2, :], iteration=self.iteration, num_queries=self.n_queries))
    ...
    def update_clustering(self, clustering):
        ...
        self.bokeh_doc.add_next_tick_callback(
            partial(update_clustering, querier=self, bokeh_layout=self.bokeh_layout, bokeh_doc=self.bokeh_doc,
                    ...
                    representatives=si_representatives, queries=self.n_queries))
